...
The Paymob SDK is distributed as an .aar file. To prevent repository bloat, we use **Git LFS** to track the paymob-sdk.aar file.

During Continuous Integration (CI), the AAR is automatically pulled along with the repository code using Git LFS.

**For Local Development**: 
- Ensure you have Git LFS installed (git lfs install).
- When you clone or pull the repository, run git lfs pull to retrieve the valid paymob-sdk.aar.
- If the file is missing or corrupted locally, your build will fail. If you need to update it, download the new paymob-sdk.aar from the Paymob developer portal (https://docs.paymob.com/docs/android-sdk), place it in pp/libs/, and commit it so LFS can track the updated version.
