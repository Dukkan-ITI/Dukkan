        ...
        try {
            ...
            val prompt = request.messages.joinToString(separator = "\n") { message ->
                when (message.role) {
                    ...
                    AiRole.Model, AiRole.Assistant -> "Assistant: ${message.text}"
                    AiRole.Tool -> "Tool ${message.toolResultName.orEmpty()} result: ${message.text} ${message.toolResult?.toString().orEmpty()}"
                }
            }
            ...
            Result.failure(AiError.Unknown)
        }
    }
